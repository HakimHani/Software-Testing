...
def add_new_features():  
    ...
    for folder in data_folders:
        ...
        bag_of_words = { k:v for k,v in key_word_count.items() if v > 3 }   
...
def calculate_performance(training_data_file, testing_data_file):
    ...
    dataset = dataset.drop('412_full_path', axis=1)
    dataset = dataset.drop('411_commit_time', axis=1)
    ...
    # print(dataset.loc[dataset['500_Buggy?']==1])
    


    decision_t = DecisionTreeClassifier(random_state=None, criterion='gini', max_depth=10, max_leaf_nodes=50, min_samples_leaf=1, splitter='best')
    X_1_resampled = resample(dataset.loc[dataset['500_Buggy?']==1], n_samples=dataset.loc[dataset['500_Buggy?']==0].shape[0], random_state=1000)
    train_data = np.concatenate((dataset.loc[dataset['500_Buggy?']==0], X_1_resampled))
    train_target = np.concatenate((train_target[train_target==0], np.ones(shape=dataset.loc[dataset['500_Buggy?']==0].shape[0], dtype=np.int32)))
    train_data =  np.delete(train_data, 14, axis=1)
